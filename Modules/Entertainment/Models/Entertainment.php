...
class Entertainment extends BaseModel
{
 ...
 public static function get_latest_movie($latestMovieIdsArray)
    ...
    {
        $builder = Entertainment::selectRaw('entertainments.id,entertainments.name,entertainments.name_en,entertainments.name_ar,entertainments.description,entertainments.description_en,entertainments.description_ar,entertainments.trailer_url_type,entertainments.trailer_url,entertainments.plan_id,plan.level as plan_level,entertainments.video_upload_type,entertainments.video_url_input,entertainments.language,entertainments.IMDb_rating,entertainments.content_rating,entertainments.duration,entertainments.release_date,entertainments.is_restricted,entertainments.poster_url,entertainments.thumbnail_url,entertainments.poster_tv_url,entertainments.enable_quality,entertainments.download_status,entertainments.download_type,entertainments.download_url,entertainments.enable_download_quality,entertainments.movie_access,entertainments.price,entertainments.purchase_type,entertainments.access_duration,entertainments.discount,entertainments.available_for,entertainments.status,entertainments.enable_subtitle,(select watched_time from continue_watch where continue_watch.entertainment_id = entertainments.id and continue_watch.user_id = '.$user_id.' and continue_watch.profile_id = '.$profile_id.' and continue_watch.entertainment_type = "movie" and continue_watch.deleted_at is null order by continue_watch.updated_at desc limit 1) as watched_time,(CASE WHEN (select id from `watchlists` where `entertainment_id` = '.$movieId.' and `user_id` = '.$user_id.' and `profile_id` = '.$profile_id.' and `watchlists`.`deleted_at` is null LIMIT 1) THEN 1 ELSE 0 END) AS is_watch_list,(CASE WHEN (select id from `likes` where `entertainment_id` = '.$movieId.' and `user_id` = '.$user_id.' and `profile_id` = '.$profile_id.' and is_like = 1  and `likes`.`deleted_at` is null LIMIT 1) THEN 1 ELSE 0 END) AS is_likes,(CASE WHEN (select id from `entertainment_downloads` where `entertainment_id` = '.$movieId.' and `device_id` = "'.$device_id.'" and `user_id` = '.$user_id.' and entertainment_type = "movie" and is_download = 1  and entertainment_downloads.`deleted_at` is null LIMIT 1) THEN 1 ELSE 0 END) AS is_download,reviews.id as your_review_id,reviews.review as your_review,reviews.rating as your_review_rating,reviews.updated_at as your_review_updated_at,reviews.created_at as your_review_created_at,reviews.user_id as your_review_user_id,users.first_name as your_review_first_name,users.last_name as your_review_last_name,users.file_url as your_review_file_url,GROUP_CONCAT(egm.genre_id) as genre_ids')
        ->join('entertainment_gener_mapping as egm','egm.entertainment_id','=','entertainments.id')
        ->leftJoin('plan', 'plan.id', '=', 'entertainments.plan_id')
